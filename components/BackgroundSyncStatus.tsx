    ...
    <div className="fixed bottom-4 right-4 space-y-2">
      ...
      <div className="bg-white border rounded shadow p-3 text-sm w-80">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setOpen(!open)} className="text-blue-700 underline">
            {open ? 'Hide' : 'Details'}
          </button>
        </div>

        <div className="mt-2 flex gap-2">
          <button onClick={retryNow} className="px-3 py-1 rounded bg-blue-600 text-white">
            Retry now
          </button>
          <button onClick={() => setToasts([])} className="px-3 py-1 rounded border">
            Clear toasts
          </button>
        </div>
            ...
            {items.map((it, i) => (
              <div key={i} className="py-2">
                ...
                <div className="text-xs opacity-70">
                  retry: {it.retryCount || 0} · next:{' '}
                  {it.nextAttempt ? new Date(it.nextAttempt).toLocaleTimeString() : 'now'}
                </div>
              </div>
            ))}
